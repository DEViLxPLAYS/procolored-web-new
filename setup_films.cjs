...
productsData.forEach(p => {
  const content = `import ConsumableProduct from '../components/ConsumableProduct';
  ...
  return (
    <ConsumableProduct
      data={{
        ...
        details: {
          outlined: \`${p.outlined.replace(/`/g, '\\`')}\`,
          description: \`${p.desc.replace(/`/g, '\\`')}\`,
          compatibility: \`${p.compat.replace(/`/g, '\\`')}\`,
          technical: \`${p.tech.replace(/`/g, '\\`')}\`
        }
      }}
    />
  );
...
const importsToAdd = productsData.map(p => `import ${p.component} from './pages/${p.component}';`).join('\n');
const routesToAdd = productsData.map(p => `              <Route path="products/${p.route}" element={<${p.component} />} />`).join('\n');

if (!appStr.includes(productsData[0].component)) {
  appStr = appStr.replace(/import UVPrinterInk500ml from '.\/pages\/UVPrinterInk500ml';/, `import UVPrinterInk500ml from './pages/UVPrinterInk500ml';\n${importsToAdd}`);
  appStr = appStr.replace(/<Route path="products\/uv-printer-ink-500ml" element={<UVPrinterInk500ml \/>} \/>/, `<Route path="products/uv-printer-ink-500ml" element={<UVPrinterInk500ml />} />\n${routesToAdd}`);
  fs.writeFileSync('src/App.tsx', appStr, 'utf8');
  console.log('App.tsx updated');
}

// 3. Append to products.ts
let prodStr = fs.readFileSync('src/data/products.ts', 'utf8');
if (!prodStr.includes(productsData[0].id)) {
  let entries = productsData.map(p => `  {
    "id": "${p.id}",
    "title": "${p.title}",
    ...
    "filters": {
      ...
      "consumablesCategory": "${p.category}"
    }
  }`).join('\n  ,\n');

  prodStr = prodStr.replace(/]\s*;\s*$/, `  ,\n${entries}\n];`);
  fs.writeFileSync('src/data/products.ts', prodStr, 'utf8');
  console.log('products.ts updated');
}

// 4. Update Collections.tsx newIds array
let collStr = fs.readFileSync('src/pages/Collections.tsx', 'utf8');
const existingArrayMatch = collStr.match(/const newIds = \\[(.*?)\\];/s);
if (existingArrayMatch) {
  const innerList = existingArrayMatch[1];
  const newAdditions = productsData.map(p => `'${p.id}'`).join(', ');
  if (!innerList.includes(productsData[0].id)) {
    const finalList = innerList.trim().endsWith(',') ? innerList + ' ' + newAdditions : innerList + ', ' + newAdditions;
    collStr = collStr.replace(/const newIds = \[(.*?)\];/s, `const newIds = [${finalList}];`);
    fs.writeFileSync('src/pages/Collections.tsx', collStr, 'utf8');
    console.log('Collections.tsx newIds updated');
  }
}
