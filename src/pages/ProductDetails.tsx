...
export default function ProductDetails() {
  ...
  const faqs = [
    ...
    { question: "What is your after-sales warranty policy, where can I get tech support?", answer: "We offer a 12-month warranty on non-ink contacting components and 6 months on the printhead (limited to one replacement). Tech support is available via support@procollored.com." },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          <div>
            ...
            <div className="flex items-end gap-3 mb-2">
              {product.price && <span className="text-3xl font-bold text-red-600">{formatConverted(convertPrice(product.price, currency.divisor))}</span>}
              {product.originalPrice && (
                 <span className="text-lg font-medium text-gray-400 line-through mb-1">{formatConverted(convertPrice(product.originalPrice, currency.divisor))}</span>
              )}
            </div>
            ...
            {product.badge && product.originalPrice && (
              <p className="text-sm font-semibold text-red-600 mb-6 flex items-center gap-1">
                ...
                <span className="text-gray-500 font-medium ml-1">
                  (Save {formatConverted(convertPrice(product.originalPrice, currency.divisor) - convertPrice(product.price, currency.divisor))})
                </span>
              </p>
            )}
